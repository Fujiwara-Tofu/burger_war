...
class TofuBot():
    ...
    def warstateCallback(self, data):
        ...
        #data_list = re.split(",|\|\"|:|{|}",str(data))
        #data_list = str(data).splitlines()
        data_list = str_data.split(',')
        for i in range(len(data_list)):
            rospy.loginfo("War State {} : {}".format(i, data_list[i]))
        '''
    ...
    def getBotPosition(self):
        self.bot_pos_x = self.odom_pose_x 
        self.bot_pos_y = self.odom_pose_y
        angle = self.odom_orientation_yaw
        max_loop = 10
    ...
    def rotate(self,goal_th,direction = 0,tolrerance_th = (10*PI/180)):
        twist = Twist()
        max_loop = 10
        Decel_range = (20*PI/180)
        
        self.getBotPosition()
        CCW_th = goal_th - self.bot_dir_th

        for i in range(max_loop):
            if (0 <= CCW_th and CCW_th < 2*PI):
                break
            elif CCW_th < 0:
                CCW_th += 2*PI
            elif CCW_th >= 2*PI:
                ...
                rospy.logerr("CCW_th calculation failure")
        else:
            rospy.logerr("CCW_th calculation loop reach to max : {}".format(CCW_th))
            CCW_th = PI
        ...
        if direction == 1: 
            #CW
            if (CW_th < tolrerance_th):           
                twist.angular.z = 0
                self.vel_pub.publish(twist)
                return 1
            elif (CW_th < Decel_range):           
                twist.angular.z = -(BURGER_MAX_ROTATE_V / Decel_range) * CW_th
            else:
                twist.angular.z = -BURGER_MAX_ROTATE_V
        elif direction == -1:
            #CCW
            if (CCW_th < tolrerance_th):           
                twist.angular.z = 0
                self.vel_pub.publish(twist)
                return 1
            elif (CCW_th < Decel_range):           
                twist.angular.z = (BURGER_MAX_ROTATE_V / Decel_range) * CCW_th
            else:
                twist.angular.z = BURGER_MAX_ROTATE_V
    ...
    def Rotue_Strategy_1_redside(self, state):        
        if state == 0:
            ...
            return self.MoveToFieldMarker(CORNER_BLOCK_4, MARKER_DOWN, 0.2)
        elif state == 2:
            return self.MoveToFieldMarker(CORNER_BLOCK_4, MARKER_DOWN)
        elif state == 3:
            return self.setGoal(self.odom_pose_x, self.odom_pose_y,PI)
        elif state == 4:
